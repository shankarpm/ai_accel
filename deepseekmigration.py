...
# Installing Packages
def install_package(project_path):
    original_cwd = os.getcwd()
    if not os.path.isdir(project_path.strip()):
    ...
    pkg_install = subprocess.run(
            "npm install --force",  # Run as a string when shell=True
...
def split_error_log_by_file(error_log: str):
    ...
    for line in error_log.splitlines():
        match = match_1 = re.search(r'(src[\\/])?app[\\/][\w./-]+?\.(ts|html|css|ngtypecheck\.ts)', line)
        if match:
            current_file = match.group().replace("\\", "/")
...
project_input = st.text_input("🔍 Enter the Project Path:", value=st.session_state.project_path)

if project_input.strip():
    st.session_state.project_path = project_input.strip()
    st.success(f"Using Project Path: {st.session_state.project_path}")

    if not st.session_state.get("packages_installed", False) and not st.session_state.get("skip_install", False):
        with st.spinner("Installing Packages..."):
            install_package(st.session_state.project_path)
...
# Run Angular build automatically if project path changes
    ...
    if st.session_state.stage == "code_fixed":
        if st.button("🔁 Start Over"):
            ...
            st.session_state.stage = "initial"
            st.session_state.skip_install = True
            st.rerun()
